...
Template.registerHelper("progress", function progress() {
  let p = Cloudinary.collection
    .find({ percent_uploaded: { $lt: 100 } })
    .fetch();
...
Template.registerHelper("editing_post", () => {
  return Session.get("editing_post");
});
Template.registerHelper("editing_user", () => {
  return Session.get("editing_user");
});
Template.registerHelper("previewing_post", () => {
  return Session.get("previewing_post");
});
Template.registerHelper("commenting_post", () => {
  return Session.get("commenting_post");
});
